...
const getUserFromStorage = () => {
  ...
  return { username: "User", role: "", profilePicture: null };
};

export const Header = () => {
  const navigate = useNavigate();
  const [username, setUsername] = useState(() => getUserFromStorage().username);
  const [role, setRole] = useState(() => getUserFromStorage().role);
  const [profilePicture, setProfilePicture] = useState(
    () => getUserFromStorage().profilePicture
  );

  // Listen for custom event to update profile picture in real-time
  useEffect(() => {
  ...
  return (
    // UBAH 1: Tambahkan 'h-16' (tinggi fix) dan 'flex items-center'
    // 'sticky top-0 z-30' agar header tetap nempel di atas saat discroll (opsional tapi bagus)
    <div className="bg-white border-b border-gray-200 shadow-sm h-16 flex items-center sticky top-0 z-30 w-full">
      {/* UBAH 2: Hapus 'py-2', ganti jadi 'w-full' agar konten rata tengah secara vertikal otomatis karena parent flex */}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <div className="text-right">
              <p className="text-sm font-semibold text-gray-800 group-hover:text-[#1a509a] transition-colors leading-tight">
                {username}
              </p>
              {role && (
                <p className="text-[10px] text-gray-500 group-hover:text-[#1a509a] transition-colors uppercase font-bold tracking-wide">
                  {role}
                </p>
              )}
            </div>
            {profilePicture ? (
              <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-[#1a509a] group-hover:shadow-md group-hover:scale-105 transition-all">
                <img
                  src={profilePicture}
                  alt={username}
